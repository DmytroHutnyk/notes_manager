
-- Insert Users
INSERT INTO users (username, password, email, is_enable) VALUES
                                                             ('user1', '{noop}password1', 'user1@example.com', TRUE),
                                                             ('user2', '{noop}password2', 'user2@example.com', TRUE),
                                                             ('admin', '{noop}adminpassword', 'admin@example.com', TRUE);



-- Insert Statuses
INSERT INTO status (name) VALUES
    ('Open'),
('In Progress'),
('Completed'),
('Archived');

-- Insert Notes
INSERT INTO note (title, description, finish_date, user_id) VALUES
('Note 1 Title', 'This is the description for Note 1', '2025-01-10', 1),
('Note 2 Title', 'This is the description for Note 2', NULL, 2);


-- Assign Statuses to Notes
INSERT INTO note_status (note_id, status_id) VALUES
    (1, 1),
(2, 2);
